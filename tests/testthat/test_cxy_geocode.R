...
test_that("misspecified functions return errors", {
  expect_error(cxy_geocode(address = street_address, city = city, state = state, dir = tdir),
  ...
  expect_error(cxy_geocode(x, address = street_address, city = city, zip = postal_code, state = state,
                           timeout = "ham", dir = tdir),
               "A numeric value must be specified for 'timeout'.")
  expect_error(cxy_geocode(x, address = street_address, city = city, state = state, zip = postal_code, output = "ham", dir = tdir),
               "Please choose one of 'tibble' or 'sf' for 'output'.")
  expect_error(cxy_geocode(x, address = street_address, city = city, state = state, zip = postal_code, style = "ham", dir = tdir),
...
test_that("warning of ommission triggers for city, state, or zip", {
  ...
  expect_warning(cxy_geocode(x, address = street_address, city = city, state = state, dir = tdir),
                 "Omission of city, state or zip code greatly reduces the speed and accuracy of the geocoder")
  expect_warning(cxy_geocode(x, address = street_address, city = city, zip = postal_code, dir = tdir),
                 "Omission of city, state or zip code greatly reduces the speed and accuracy of the geocoder")
  expect_warning(try(cxy_geocode(x, address = street_address, state = state, zip = postal_code, dir = tdir), silent = TRUE),
                 "Omission of city, state or zip code greatly reduces the speed and accuracy of the geocoder")
})
...
test_that("output type produces correct output class", {
  ...
  expect_s3_class(cxy_geocode(x, address = street_address, city = city, state = state, zip = postal_code,
                              output = "sf", style = "minimal", dir = tdir), "sf")
  expect_s3_class(cxy_geocode(x, address = "street_address", city = "city", state = "state", zip = "postal_code",
                              output = "tibble", style = "full", dir = tdir), "tbl_df")
})
...
test_that("error is returned for zero matches", {
  skip_on_cran()
  skip_if_offline(host = "r-project.org")
  expect_error(suppressWarnings(cxy_geocode(x, address = "street_address", dir = tdir)),
